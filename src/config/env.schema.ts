...
export const envSchema = z
  .object({
    ...
    CORS_ORIGINS: z
      .preprocess(
        ...
        z
          .array(z.string().url())
          .min(1, 'CORS_ORIGINS requiere al menos 1 origen valido'),
      )
    ...
    CORS_ALLOWED_METHODS: z
      .preprocess(
        ...
        z
          .array(z.string().min(1))
          .min(1, 'CORS_ALLOWED_METHODS requiere al menos 1 metodo'),
      )
    ...
    AUTH_REFRESH_COOKIE_SAME_SITE: z.preprocess(
      parseOptionalString,
    ...
    REMINDERS_RETRY_DELAY_MINUTES: z.coerce
      .number()
      .min(1)
      .max(1440)
      .default(15),
    REMINDERS_PROCESSING_STALE_MINUTES: z.coerce
      .number()
      .min(1)
      .max(1440)
      .default(10),
  ...
  .superRefine((value, ctx) => {
    if (value.TURNSTILE_ENABLED && !value.TURNSTILE_SECRET_KEY?.trim()) {
      ctx.addIssue({
